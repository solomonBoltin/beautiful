...
def mark(u=30, gap=0.5, arch_h=3.6, sq=0.8, scale=0.7, dx=0.0, dy=0.0, size=1024, radius=0.22, bar=True, bar_gap=0.5):
    """Everything in units of u. i: stem u wide, dot u square, half-unit gap; half-unit gap to the arch; arch 5u wide,
    ...
    x0 = 0; top = 0; base = ih
    g.append(f'<rect x="{x0}" y="{top}" width="{u}" height="{u}" rx="{u*0.27:.1f}" fill="{INK}"/>')
    g.append(f'<rect x="{x0}" y="{top + 1.5*u}" width="{u}" height="{ih - 1.5*u}" rx="{u*0.27:.1f}" fill="{INK}"/>')
    ax = x0 + u + gap * u; aw = 5 * u; R_out = 1.8 * u; R_in = 0.8 * u; leg_r = 0.2 * u
    ...
    for i in range(2):
        for j in range(2):
            g.append(f'<rect x="{ax + u + pitch + i*(s + pitch):.1f}" y="{base - s - j*(s + pitch):.1f}" width="{s:.1f}" height="{s:.1f}" rx="{s*0.2:.1f}" fill="{INK}"/>')
    w = u + gap * u + aw; h = ih
    if bar:
        by = base + bar_gap * u; g.append(f'<rect x="0" y="{by:.1f}" width="{w:.1f}" height="{u}" rx="{u/2}" fill="{MINT}"/>'); h = by + u
